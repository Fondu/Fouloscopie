...
target_caption = text(-1.5, -1.1, sprintf('Energie de la cible : %0.1f%%', target_energy), 'FontSize', 10, 'FontWeight', 'bold', 'Color','r');
time_caption = text(-1.5, -1.2, sprintf('Temps écoulé : 0 s'), 'FontSize', 10, 'FontWeight', 'bold', 'Color','r');
robots_caption = text(-1.5, -1.3, sprintf('Nb de robots attaquants : 0 (0)'), 'FontSize', 10, 'FontWeight', 'bold', 'Color','r');
uistack(target_caption, 'top'); 
uistack(time_caption, 'top'); 
uistack(robots_caption, 'top'); 
uistack(d, 'bottom');
...
while target_energy>0 && total_time<900
   ...
   % UPdate total time
            ...
            % Collect local information for robot i
                ...
                % Target information (when applicable)
                    if (d_target(i)<DETECTION_RANGE)
                        allRobots{i}.set_info_cible(x_target, y_target, 1);
                    end
    ...
    % Display
        ...
        robots_caption.String = sprintf('Nombre de robots attaquants : %d (%d)', nbAttaquants, nbDetected);
